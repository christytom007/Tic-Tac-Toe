...
public class TicTacToeModel implements IGameModel
{
	 ...
	 */
	public TicTacToeModel(IGameView view, GameMode gameMode, int gameSize, String player1Name, String player2Name, PlayerSymbol player1,
			PlayerSymbol player2, GameLogFileHandler logFileHandler)
	{
		this.view = view;
	...
	public void reset()
	{
		...
		logFileHandler.writeLog("------------------------------------------------------------------------------");
		logFileHandler.writeLog("Game Resetted.");

		randomizeTurn();

		view.updateTurn(currentPlayer);

		if (gameMode == GameMode.LOCAL_PVC && currentPlayer == player2) advancedAITurn();
	}
	...
	/**
	 * Advanced AI Algorithum with simple logical calculations
	 */
	private void advancedAITurn()
	{
		...
		for (int row = 0; row < gameSize - 2; row++)
		{
			...
			{
				if (grid[row][column] == player2 && grid[row + 1][column] == Square.Empty && grid[row + 2][column] == player2)
				{// Vertical
					makeMove(row + 1, column);
					return;
				}
				else if (grid[row][column] == Square.Empty && grid[row + 1][column] == player2 && grid[row + 2][column] == player2)
				{
					makeMove(row, column);
					return;
				}
				else if (grid[row][column] == player2 && grid[row + 1][column] == player2 && grid[row + 2][column] == Square.Empty)
				{
		...
		for (int row = 0; row < gameSize; row++)
		{
			...
			{
				if (grid[row][column] == player2 && grid[row][column + 1] == Square.Empty && grid[row][column + 2] == player2)
				{// Horizontal
					makeMove(row, column + 1);
					return;
				}
				else if (grid[row][column] == Square.Empty && grid[row][column + 1] == player2 && grid[row][column + 2] == player2)
				{// Horizontal
					makeMove(row, column);
					return;
				}
				else if (grid[row][column] == player2 && grid[row][column + 1] == player2 && grid[row][column + 2] == Square.Empty)
				{// Horizontal
		...
		for (int row = 0; row < gameSize - 2; row++)
		{
			for (int column = 0; column < gameSize - 2; column++)
			{
				if (grid[row][column] == player2 && grid[row + 1][column + 1] == Square.Empty && grid[row + 2][column + 2] == player2)
				{// Diagonal "\"
					makeMove(row + 1, column + 1);
					return;
				}
				else if (grid[row][column] == Square.Empty && grid[row + 1][column + 1] == player2 && grid[row + 2][column + 2] == player2)
				{// Diagonal "\"
					makeMove(row, column);
					return;
				}
				else if (grid[row][column] == player2 && grid[row + 1][column + 1] == player2 && grid[row + 2][column + 2] == Square.Empty)
				{// Diagonal "\"
		...
		for (int row = 0; row < gameSize - 2; row++)
		{
			for (int column = 0; column < gameSize; column++)
			{
				if (grid[row][column] == player1 && grid[row + 1][column] == Square.Empty && grid[row + 2][column] == player1)
				{// Vertical
					makeMove(row + 1, column);
					return;
				}
				if (grid[row][column] == Square.Empty && grid[row + 1][column] == player1 && grid[row + 2][column] == player1)
				{// Vertical
					makeMove(row, column);
					return;
				}
				if (grid[row][column] == player1 && grid[row + 1][column] == player1 && grid[row + 2][column] == Square.Empty)
				{// Vertical
					makeMove(row + 2, column);
					return;
				}
		...
		for (int row = 0; row < gameSize; row++)
		{
			for (int column = 0; column < gameSize - 2; column++)
			{
				if (grid[row][column] == player1 && grid[row][column + 1] == Square.Empty && grid[row][column + 2] == player1)
				{// Horizontal
					makeMove(row, column + 1);
					return;
				}
				else if (grid[row][column] == Square.Empty && grid[row][column + 1] == player1 && grid[row][column + 2] == player1)
				{// Horizontal
					makeMove(row, column);
					return;
				}
				else if (grid[row][column] == player1 && grid[row][column + 1] == player1 && grid[row][column + 2] == Square.Empty)
				{// Horizontal
					makeMove(row, column + 2);
					return;
				}
		...
		for (int row = 0; row < gameSize - 2; row++)
		{
			for (int column = 0; column < gameSize - 2; column++)
			{
				if (grid[row][column] == player1 && grid[row + 1][column + 1] == Square.Empty && grid[row + 2][column + 2] == player1)
				{// Diagonal "\"
					makeMove(row + 1, column + 1);
					return;
				}
				else if (grid[row][column] == Square.Empty && grid[row + 1][column + 1] == player1 && grid[row + 2][column + 2] == player1)
				{// Diagonal "\"
					makeMove(row, column);
					return;
				}
				else if (grid[row][column] == player1 && grid[row + 1][column + 1] == player1 && grid[row + 2][column + 2] == Square.Empty)
				{// Diagonal "\"
					makeMove(row + 2, column + 2);
					return;
				}
	...
	private boolean findWinner(Square currentPlayer)
	{
		for (int row = 0; row < gameSize - 2; row++)
		{
			for (int column = 0; column < gameSize; column++)
			{
				if (grid[row][column] == currentPlayer && grid[row + 1][column] == currentPlayer && grid[row + 2][column] == currentPlayer)
				{// Vertical++++9*
					view.updateWinningStrikes(row, column, row + 1, column, row + 2, column);
					logFileHandler.writeLog(currentPlayer + " got a Strike in [" + row + "," + column + "],[" + (row + 1) + "," + column + "],["
							+ (row + 2) + "," + column + "] position.");
					addScore();
					hasWinner = true;
				}
			}
		...
		for (int row = 0; row < gameSize; row++)
		{
			for (int column = 0; column < gameSize - 2; column++)
			{
				if (grid[row][column] == currentPlayer && grid[row][column + 1] == currentPlayer && grid[row][column + 2] == currentPlayer)
				{// Horizontal
					view.updateWinningStrikes(row, column, row, column + 1, row, column + 2);
					logFileHandler.writeLog(currentPlayer + " got a Strike in [" + row + "," + column + "],[" + row + "," + (column + 1) + "],[" + row
							+ "," + (column + 2) + "] position.");
					addScore();
					hasWinner = true;
				}
		...
		for (int row = 0; row < gameSize - 2; row++)
		{
			for (int column = 0; column < gameSize - 2; column++)
			{
				if (grid[row][column] == currentPlayer && grid[row + 1][column + 1] == currentPlayer && grid[row + 2][column + 2] == currentPlayer)
				{// Diagonal "\"
					view.updateWinningStrikes(row, column, row + 1, column + 1, row + 2, column + 2);
					logFileHandler.writeLog(currentPlayer + " got a Strike in [" + row + "," + column + "],[" + (row + 1) + "," + (column + 1) + "],["
							+ (row + 2) + "," + (column + 2) + "] position.");
					addScore();
					hasWinner = true;
				}
			}
	...
	{
		if (grid[x][y] != Square.Empty || hasWinner) return;
		...
		if (findWinner(grid[x][y])) return; // findWinner return true if the meathod Find that Game is complete, so that we
